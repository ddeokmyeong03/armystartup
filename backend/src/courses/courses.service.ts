...
export class CoursesService {
  ...
  async recommend(userId: number) {
    ...
    // 3단계: 기존 추천 삭제 후 Claude 추천 생성
    await this.prisma.courseRecommendation.deleteMany({ where: { userId, status: 'RECOMMENDED' } });

    let aiResults: Array<{ courseId: number; reason: string; priority: number }> = [];

    try {
      const prompt = `사용자 목표: ${goals.length > 0 ? goals.map((g) => `${g.title}(${g.type})`).join(', ') : '목표 없음'}
하루 가용 학습 시간: ${availableMinutes}분

추천 대상 강의 목록:
${candidates.map((c) => `- ID:${c.id} | ${c.title} | ${c.source} | ${c.durationMinutes}분 | 태그: ${JSON.parse(c.tags).join(', ')}`).join('\n')}

위 강의 중 사용자 목표와 학습 시간에 가장 적합한 강의 최대 3개를 선별하고 추천 이유를 작성해주세요.
반드시 제공된 강의 목록에 있는 강의만 추천하세요.
JSON 배열로만 응답하세요: [{"courseId": number, "reason": "추천 이유 (2-3문장)", "priority": 1}]`;

      const response = await this.anthropic.messages.create({
        model: 'claude-haiku-4-5-20251001',
        max_tokens: 600,
        system: '당신은 군 병사의 자기개발을 돕는 AI 가이드 밀로그입니다. 장병e음, 국방전직교육원 강의를 중심으로 추천합니다. JSON 배열로만 응답하세요.',
      ...
      // Claude 실패 시 규칙 기반으로 fallback
      aiResults = candidates.slice(0, 3).map((c, i) => ({
        courseId: c.id,
