...
const PasswordPage = () => {
  ...
  if (done) {
    return (
      <div className="page page-enter" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center', padding: '0 40px' }}>
          <div style={{ fontSize: 64, marginBottom: 20 }}>🔐</div>
          <div className="t-title">비밀번호 변경 완료</div>
          <div className="t-subdued" style={{ marginTop: 10, lineHeight: 1.6 }}>
            다음 로그인 시 새 비밀번호를 사용하세요.
          </div>
          <button className="btn btn-primary btn-full" style={{ marginTop: 36, height: 52 }} onClick={back}>확인</button>
        </div>
      </div>
    );
  }
    ...
    <div className="page page-enter">
      ...
      <div className="scroll-area" style={{ padding: '16px 20px 48px', position: 'relative', zIndex: 1 }}>
        ...
        <button
          ...
          style={{ marginTop: 32, height: 52, opacity: valid ? 1 : 0.38, transition: 'opacity 200ms' }}
    ...
    <div>
      ...
      <div style={{ position: 'relative' }}>
        ...
        <button onClick={() => setShow(s => !s)} style={{
          ...
          color: 'var(--text-subdued)', fontSize: 12, fontWeight: 700,
        }}>
          {show ? '숨김' : '표시'}
        </button>
      </div>
      {hint && (
        <div style={{ marginTop: 6, fontSize: 12, fontWeight: 600, color: hintOk ? 'var(--accent)' : '#ef4444' }}>{hint}</div>
      )}
    </div>
  );
}
...
Object.assign(window, { PasswordPage });
